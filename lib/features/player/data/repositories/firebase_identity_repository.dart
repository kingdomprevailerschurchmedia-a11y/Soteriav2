...
class FirebaseIdentityRepository implements IdentityRepository {
  ...
  FirebaseIdentityRepository(this._firestore);
  ...
  Future<PublicCompetitiveProfile?> getPublicProfile(String userId) async {
    final doc = await _firestore.collection('public_profiles').doc(userId).get();
    if (!doc.exists) return null;
    return PublicProfileDto.fromFirestore(doc);
  }
  ...
  Future<List<PublicCompetitiveProfile>> searchPlayers(String query, {int limit = 20}) async {
    ...
    return snapshot.docs
        .map((doc) => PublicProfileDto.fromFirestore(doc))
        .toList();
  }
}
