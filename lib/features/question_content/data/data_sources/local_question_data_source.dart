...
class LocalQuestionDataSource {
  ...
  static const String _competitiveBankPath = 'assets/data/competitive_questions.json';
  ...
  /// Loads the competitive/pro questions from local assets.
  Future<List<QuestionDto>> fetchCompetitiveQuestions() async {
    try {
      final String jsonString = await rootBundle.loadString(_competitiveBankPath);
      final List<dynamic> jsonList = json.decode(jsonString);
      return jsonList.map((json) => QuestionDto.fromJson(json as Map<String, dynamic>)).toList();
    } catch (e) {
      return [];
