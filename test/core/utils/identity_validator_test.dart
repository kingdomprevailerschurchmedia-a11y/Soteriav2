...
void main() {
  group('IdentityValidator', () {
    ...
    test('isValidUsername validates username rules', () {
      ...
      expect(IdentityValidator.isValidUsername('user name'), false); // No spaces
    });

    test('getPasswordStrength calculates correctly', () {
      expect(IdentityValidator.getPasswordStrength(''), 0.0);
      expect(IdentityValidator.getPasswordStrength('password'), 0.4); // min length + lowercase
      expect(IdentityValidator.getPasswordStrength('Password123!'), 1.0); // all met
    });
  });
}
